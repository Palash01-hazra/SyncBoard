...
const Sidebar = () => {
  ...
  const fetchCanvases = async () => {
    try {
      const response = await axios.get('https://syncboard-backend-7zx2.onrender.com/api/canvas/list', {
  ...
  const handleCreateCanvas = async () => {
    try {
      const response = await axios.post('https://syncboard-backend-7zx2.onrender.com/api/canvas/create', {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log(response.data)  
      fetchCanvases();
  ...
  const handleDeleteCanvas = async (id) => {
    try {
      await axios.delete(`https://syncboard-backend-7zx2.onrender.com/api/canvas/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCanvases();
